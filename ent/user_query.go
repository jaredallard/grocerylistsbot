...
type UserQuery struct {
	...
	withActiveList  *GroceryListQuery
	withFKs         bool
...
func (uq *UserQuery) QueryGrocerylist() *GroceryListQuery {
	query := &GroceryListQuery{config: uq.config}
...
// QueryActiveList chains the current query on the active_list edge.
func (uq *UserQuery) QueryActiveList() *GroceryListQuery {
	query := &GroceryListQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (uq *UserQuery) WithGrocerylist(opts ...func(*GroceryListQuery)) *UserQuery {
	query := &GroceryListQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withGrocerylist = query
	return uq
}

//  WithActiveList tells the query-builder to eager-loads the nodes that are connected to
// the "active_list" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithActiveList(opts ...func(*GroceryListQuery)) *UserQuery {
	query := &GroceryListQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withActiveList = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withGrocerylist; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.GrocerylistPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "grocerylist": %v`, err)
		}
		query.Where(grocerylist.IDIn(edgeids...))
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "grocerylist" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Grocerylist = append(nodes[i].Edges.Grocerylist, n)
			}
		}
	}

	if query := uq.withActiveList; query != nil {
