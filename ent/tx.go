...
func (tx *Tx) init() {
	tx.GroceryItem = NewGroceryItemClient(tx.config)
	tx.GroceryList = NewGroceryListClient(tx.config)
	tx.User = NewUserClient(tx.config)
	tx.UserIDMapping = NewUserIDMappingClient(tx.config)
}
...
// applies a query, for example: GroceryItem.QueryXXX(), the query will be executed
